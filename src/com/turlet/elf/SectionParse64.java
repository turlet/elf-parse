...
public class SectionParse64 {
    ...
    private List<ElfSectionDynsym64> dynsymList;

    //.rodata保存了只读数据，可以读取但不能修改，例如printf语句中的所有静态字符串封装到该节。
    private List<String> rodataList;

    public SectionParse64(FileChannel fileChannel, List<ElfSectionHeader64> sectionHeaders) {
    ...
    public void parseDymStr() throws IOException {
        List<ElfSectionHeader64> dynstrList = sectionHeaders.stream().filter(header -> ".dynstr\u0000".equalsIgnoreCase(header.name)).collect(Collectors.toList());
        if (dynstrList.isEmpty()) {
            Log.e("找不到对应的Section的dynstr字符串表");
            return;
        }
        ElfSectionHeader64 dynstrHeader = dynstrList.get(0);

        fileChannel.position(dynstrHeader.sh_offset);
        dynstrBuf = ByteBuffer.allocate(Long.valueOf(dynstrHeader.sh_size).intValue());
    ...
    public void parseDynSym() throws IOException {
        List<ElfSectionHeader64> sectionDynSymList = sectionHeaders.stream().filter(header -> ".dynsym\u0000".equalsIgnoreCase(header.name)).collect(Collectors.toList());
        if (sectionDynSymList.isEmpty()) {
            Log.e("找不到对应的Section的dynsym符号表");
            return;
        }
        ElfSectionHeader64 dynsymHeader = sectionDynSymList.get(0);

        fileChannel.position(dynsymHeader.sh_offset);
        ByteBuffer dynsymBuf = ByteBuffer.allocate(Long.valueOf(dynsymHeader.sh_size).intValue());
        fileChannel.read(dynsymBuf);
        dynsymBuf.rewind();

        int size = (int)dynsymHeader.sh_size /(int)dynsymHeader.sh_entsize;
        dynsymList = new ArrayList<>(size);
        ElfSectionDynsym64 dynsym;
        for (int i = 0; i < size; i++) {
            Elf64.Elf64_Sym sym = new Elf64.Elf64_Sym();
            ...
            dynsym = ElfSectionDynsym64.parse(sym);
        ...
        ElfSectionDynsym64.printTableTitle(dynsymList.size());
        dynsymList.stream().forEach(ElfSectionDynsym64::print);
    ...
    public void parseHash() throws IOException {
        List<ElfSectionHeader64> hashList = sectionHeaders.stream().filter(header -> ".hash\u0000".equalsIgnoreCase(header.name)).collect(Collectors.toList());
        if (hashList.isEmpty()) {
            Log.e("找不到对应的Section的.hash节");
            return;
        }
        ElfSectionHeader64 hashHeader = hashList.get(0);

        fileChannel.position(hashHeader.sh_offset);
        ByteBuffer dynsymBuf = ByteBuffer.allocate(Long.valueOf(hashHeader.sh_size).intValue());
        fileChannel.read(dynsymBuf);
        dynsymBuf.rewind();
    }
    ...
    public void parseRodata() throws IOException {
        List<ElfSectionHeader64> rodataHeaderList = sectionHeaders.stream().filter(header -> ".rodata\u0000".equalsIgnoreCase(header.name)).collect(Collectors.toList());
        if (rodataHeaderList.isEmpty()) {
            Log.e("找不到对应的Section的rodata节");
            return;
        }
        ElfSectionHeader64 rodataHeader = rodataHeaderList.get(0);

        fileChannel.position(rodataHeader.sh_offset);
        ByteBuffer rodataBuf = ByteBuffer.allocate(Long.valueOf(rodataHeader.sh_size).intValue());
    ...
    public void parseNote() throws IOException {
        List<ElfSectionHeader64> noteHeaderList = sectionHeaders.stream().filter(header -> header.name.startsWith(".note")).collect(Collectors.toList());
        if (noteHeaderList.isEmpty()) {
            Log.e("找不到对应的Section的note");
            return;
        }
        ElfSectionHeader64 noteHeader = noteHeaderList.get(0);

        fileChannel.position(noteHeader.sh_offset);
        ByteBuffer noteBuf = ByteBuffer.allocate(Long.valueOf(noteHeader.sh_size).intValue());
        fileChannel.read(noteBuf);
        noteBuf.rewind();

        Elf64.Elf64_Nhdr nhdr = new Elf64.Elf64_Nhdr();
        noteBuf.get(nhdr.n_namesz);
        noteBuf.get(nhdr.n_descsz);
        noteBuf.get(nhdr.n_type);

        ElfSectionNotes64 notes = ElfSectionNotes64.parse(nhdr);
        ...
        ElfSectionNotes64.printTableTitle(noteHeader.sh_offset, Long.valueOf(noteHeader.sh_size).intValue());
        notes.print();
    }
    ...
    public void parseComment() throws IOException{
        List<ElfSectionHeader64> commentHeaderList = sectionHeaders.stream().filter(header -> ".comment\u0000".equalsIgnoreCase(header.name)).collect(Collectors.toList());
        if (commentHeaderList.isEmpty()) {
            Log.e("找不到对应的Section的comment节");
            return;
        }
        ElfSectionHeader64 commentHeader = commentHeaderList.get(0);

        fileChannel.position(commentHeader.sh_offset);
        ByteBuffer noteBuf = ByteBuffer.allocate(Long.valueOf(commentHeader.sh_size).intValue());
        fileChannel.read(noteBuf);
        noteBuf.rewind();
